...
- **Deferring a recheck to a future date:** if this task (or a comment on it) calls for repeating the same check again later (e.g. "recheck in a few days", a periodic health check) rather than right now, do **not** guess whether enough time has passed and do not file a separate backlog task just to hold the wait. Instead add `Recheck-after: <YYYY-MM-DD>` to *this* task's own description (`next-task` skips a Todo task carrying an unreached future date the same way it skips a `Blocked by:` reference) and end the iteration with `<promise>TASK_BLOCKED</promise>` — the loop moves it back to Todo, where it stays invisible to `next-task` until that date arrives, then gets re-picked on its existing branch/PR (or with none, if the original check needed no repo changes — see _Iteration detection_). If more than one `Recheck-after:` line ends up in the description, the last one wins, same as `Model:`/`Effort:`.
