...
You never call state-transition commands yourself — signal the outcome via the promise markers in step 9 and the loop performs the actual transition. Do **not** call `set-in-progress`, `set-review`, `set-todo`, `set-done`, or `set-cancelled`.
...
- **Deferring a recheck to a future date or time:** if this task (or a comment on it) calls for repeating the same check again later (e.g. "recheck in a few days", "check again in 30 minutes", a periodic health check) rather than right now, do **not** guess whether enough time has passed and do not file a separate backlog task just to hold the wait. Instead add `Recheck-after: <YYYY-MM-DD>` (day granularity — pending for the rest of today, unblocks at the start of the given day) or, for a same-day wait measured in minutes/hours, `Recheck-after: <YYYY-MM-DD>T<HH:MM>` (24-hour UTC clock) to *this* task's own description (`next-task` skips a Todo task carrying an unreached future date/time the same way it skips a `Blocked by:` reference) and end the iteration with `<promise>TASK_BLOCKED</promise>` — the loop moves it back to Todo, where it stays invisible to `next-task` until that date/time arrives, then gets re-picked on its existing branch/PR (or with none, if the original check needed no repo changes — see _Iteration detection_). Always use `T` as the date/time separator, never a space — the comparison is a plain string compare against a `T`-separated timestamp, and a space sorts before `T` and would make the recheck resolve too early. If more than one `Recheck-after:` line ends up in the description, the last one wins, same as `Model:`/`Effort:`.
- **Per-task model/effort override, `Model:`/`Effort:` — always set both on every task you create.** A task's description can contain `Model: <name>` (e.g. `Model: opus`, `Model: sonnet`, `Model: haiku`, `Model: fable`, or a full model id like `claude-sonnet-5`) and `Effort: <level>` (`low`, `medium`, `high`, `xhigh`, or `max`) to run that task on a different model/reasoning-effort than the loop's configured default. Both are read from the description before the iteration starts, so neither has any effect if added mid-iteration — only the next time the task starts (including a re-pick after `TASK_BLOCKED`). If a description ends up with more than one `Model:` or `Effort:` line, the last occurrence of each wins — this is why appending a new one (e.g. via `append-description`) overrides an earlier one rather than being ambiguous. Leaving them unset is not a neutral choice: an unset task inherits this project's own `RALPH_MODEL`/`RALPH_EFFORT`, which is normally the most capable model at a medium reasoning effort — cheaper than the max tier, but still not free. Default a new task to the cheapest pairing it can actually succeed at — `Model: haiku` + `Effort: low` for a small, mechanical, fully-specified follow-up; `Model: sonnet` + `Effort: medium` for ordinary well-structured work; reserve `Model: opus` and `Effort: high`/`xhigh`/`max` for a task that is still ambiguous, exploratory, or genuinely hard. A cheap model only succeeds when the task needs no further discovery, so when filing one, write its investigation/checklist into the new task's own description up front (the same pattern as step 3.2) rather than leaving that for the cheaper tier to figure out.
...
- **To find a non-image file attached to the task** (e.g. a zip someone attached as supporting material): `list-images`/the `images` field only cover images embedded inline in HTML — a file attached through Plane's own "Attach" UI panel is invisible to both. Run `list-attachments <id>` instead, then `download-asset <asset_id> <local_path> <id>` with the id it returns. When a task references source material (a zip, an export, etc.) with no obvious location, check `list-attachments` before concluding nothing was provided.
...
- **`OPEN`** — check out the existing branch and continue; **do not create a new branch or call `set-branch`**:
  ...
  git checkout "$BRANCH"
  ```
- **`MERGED`, `CLOSED` or `NONE`** — a new PR is needed. Create a new branch by appending `-v2` (then `-v3`, …, until unused):
  ```bash
...
**Writing "I'll wait for it" or "I'll pause here" and then producing no further tool call is the same failure as calling `ScheduleWakeup`** — a background-process-monitoring tool included in your toolset (one that "streams" or "subscribes to" a running command's output) does not except this loop from the rule above: if it does not itself block until the command is finished and hand back the final result inside this same turn, using it (or narrating an intention to use it) still ends the turn with the work unfinished and no one left to resume it. Confirmed live (Plane task #1533): the agent backgrounded a test run, wrote "I'll wait for the background task notification instead of polling," and the process exited on that turn with no `<promise>` marker at all — the checklist items it had already implemented were left uncommitted with no branch/PR and were later overwritten by unrelated work. The only two valid ways to finish waiting on a command started this way are still: run it in the foreground so the tool call itself blocks, or self-poll it with your own bounded `sleep N && <status-check>` loop that you keep calling, in this same turn, until it reports done.
